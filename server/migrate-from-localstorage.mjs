#!/usr/bin/env node
/**
 * Migration script: Import work records from localStorage JSON to PostgreSQL Database
    ...
    for (const record of records) {
      try {
        ...
        const result = await sql`
          ...
          VALUES (${dbRecord.userId}, ${dbRecord.date}, ${dbRecord.month}, ${dbRecord.customerName}, ${dbRecord.customerPhone}, ${dbRecord.product}, ${dbRecord.os}, ${dbRecord.serviceType}, ${dbRecord.details}, ${dbRecord.notes}, ${dbRecord.status}, NOW(), NOW())
          RETURNING id
        `;

        // Import images if present
        if (record.images && Array.isArray(record.images) && result.length > 0) {
          const recordId = result[0].id;
          for (const image of record.images) {
            try {
              await sql`
                INSERT INTO workRecordImages (recordId, filename, url, uploadedAt) 
                VALUES (${recordId}, ${image.filename}, ${image.url}, ${image.uploadedAt || new Date().toISOString()})
              `;
            } catch (imgErr) {
              console.warn(`⚠️  Failed to import image for record ${recordId}:`, imgErr.message);
            }
          }
        }
